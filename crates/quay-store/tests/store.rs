...
use quay_store::migrations::{MIGRATIONS, latest_version};
...
fn opening_a_new_database_brings_it_to_the_latest_schema_version() {
    ...
    match store.schema_version() {
        Ok(version) => assert_eq!(version, latest_version(&MIGRATIONS)),
...
fn reopening_a_database_does_not_replay_the_migrations() {
    ...
    match Store::open(&path) {
        Ok(store) => assert!(
            matches!(store.schema_version(), Ok(version) if version == latest_version(&MIGRATIONS))
        ),
